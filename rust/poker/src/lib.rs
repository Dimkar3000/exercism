...
pub fn winning_hands<'a>(hands: &[&'a str]) -> Option<Vec<&'a str>> {
    ...
    for hand in hands {
        if winners.is_empty() || score(hand) == score(winners[0]) {
            winners.push(hand)
...
fn score(hand: &str) -> (usize, Vec<usize>) {
    ...
    for (r, func) in ranks.iter().enumerate().rev() {
        if let Some(score) = func(&values, &suits) {
            return (r, score);
        }
    }
    (0, Vec::new())
...
fn suit(s: &str) -> usize {
    "HSCD".split("").position(|i| i == s).unwrap()
}
...
fn straight(values: &Values, _: &Suits) -> Option<Vec<usize>> {
    ...
    let diffs = values
        .iter()
        .map(|&v| largest - (v.0 as isize))
        .collect::<Vec<isize>>();
    if diffs == &[0, 1, 2, 3, 4] {
        Some(values.iter().map(|&v| v.0).collect())
    } else if largest == 13 && diffs == [0, 9, 10, 11, 12] {
        Some(vec![0, 1, 2, 3, 4])
    } else {
        None
    }
